...
define('MFN_THEME_VERSION', '21.5.8');
...
//Kh?i t?o function cho shortcode
function create_shortcode($attr, $content = null) {
		...
		$args = array(
			...
			$args['testimonial-types'] = $category;
							...
							if( $query_tm->have_posts() ) :
								while( $query_tm -> have_posts() ) : $query_tm->the_post(); 
									$slug = get_post_field( 'post_name', get_post() );
	echo '<div class="item" id="post-'. get_the_ID() .'">';
						
									
										echo '<div class="column one-fourth img-p">';
											echo get_the_post_thumbnail(null, 'testimonials', array('class'=>'scale-with-grid'));
										echo '</div>';
										echo '<div class="ex column three-fourth">';
	
											 echo '<div class="content-ph">';
                                            echo '<div class="dcm">'. get_the_content() .'</div>';
                                            echo '<h4>'. esc_html(get_post_meta(get_the_ID(), 'mfn-post-author', true)) .'</h4>';
                                            echo '<h5>'. get_post_meta(get_the_ID(), 'mfn-post-company', true) .'</h5>';
										echo '</div>';
                                        echo '</div>';
...
add_shortcode( 'Slide_duan', 'create_shortcode' );